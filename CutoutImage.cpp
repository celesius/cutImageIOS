...
//            cv::warpAffine(colorMat, dstMat, rotMat, cv::Size(std::max(srcMat.rows,srcMat.cols),std::max(srcMat.rows,srcMat.cols)), CV_INTER_LINEAR);
            cv::warpAffine(colorMat, dstMat, rotMat, cv::Size(std::max(srcMat.rows,srcMat.cols),std::max(srcMat.rows,srcMat.cols)), CV_INTER_LINEAR);
            cv::warpAffine(wholeImage.clone(), wholeImageRotate, rotMat, cv::Size(std::max(srcMat.rows,srcMat.cols),std::max(srcMat.rows,srcMat.cols)), CV_INTER_LINEAR);
    ...
    for(int y = 0;y<rows;y++){
        ...
        for(int x = 0;x<cols;x=x+4){
            ...
            if(oneChannelData == 255)
            {
                ...
                showPicLineData[x] = showPicLineData[x + 1]/2 + 197/2;
...
//    return CutoutImage::classMergeMat;
    return CutoutImage::classMaskMat;
}
...
cv::Mat CutoutImage::scaleFCMI2InputColorImageSize( const cv::Mat srcMat )
{
    ...
    if(srcMat.rows >= srcMat.cols)  //最终图像是竖直的
    {
        ...
        if(scaleToWidth <= inputColorImageSize.width){
            cv::resize(srcMat, aMat, cv::Size(scaleToWidth,scaleToHeight), 0, 0, CV_INTER_LINEAR);
